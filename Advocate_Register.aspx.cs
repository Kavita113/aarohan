...
namespace Project_UI
{
    public partial class Advocate_Register : Page
    {
        ...
        protected void SubmitButton_Click(object sender, EventArgs e)
        {
            ...
            // File upload logic
            string bciCertificatePath = Server.MapPath("Images/"); // Declare the path variables outside the file upload block
            string photoPath = Server.MapPath("Images/");
            string bcifilename = "";
            string photographfilename = "";
            ...
            if (BCIEnrolmentCertificate.HasFile && Photograph.HasFile)
            {
                try
                {
                    ...
                    string ex = Path.GetExtension(BCIEnrolmentCertificate.FileName);
                    BCIEnrolmentCertificate.SaveAs(bciCertificatePath +BCIEnrolmentCertificate.FileName);
                     bcifilename = "Images/" + BCIEnrolmentCertificate.FileName;
                    ...
                    string ex2 = Path.GetExtension(Photograph.FileName);
                    Photograph.SaveAs(photoPath + Photograph.FileName);
                     photographfilename = "Images/" + Photograph.FileName;

                    // Check if both files are uploaded
                    if (string.IsNullOrEmpty(bcifilename) || string.IsNullOrEmpty(photographfilename))
                    {
                        isValid = false;
            ...
            (@FullName, @Email, @PhoneNumber, @State, @City, @Gender, @Password, @ConfirmPassword, @YearsOfExperience, @AreaOfSpecialization, @BCIEnrolmentCertificate, @Photograph)";
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@BCIEnrolmentCertificate", bcifilename); 
                        cmd.Parameters.AddWithValue("@Photograph", photographfilename); 
